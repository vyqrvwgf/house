...
def list(request):
    ...
    clients = Profile.objects.filter(is_del=False).order_by('-created')
    # 筛选结果集
    if param_name:
...
@staff_member_required(login_url='/admin/login')
def offline(request, profile_id):
    page = request.GET.get('page', '')
    param_name = request.GET.get('param_name', '')
    param_mobile = request.GET.get('param_mobile', '')
    param_email = request.GET.get('param_email', '')
    ...
    client = Profile.objects.filter(pk=profile_id).first()
    if client:
        client.deactivate()
